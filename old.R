...
<link rel="manifest" href="https://raw.githubusercontent.com/mudassiribrahim12/me/main/site.webmanifest">
    <meta property="og:site_name" content="The Official Website of Mudasir Mohammed Ibrahim">
<meta property="og:title" content="The Official Website of Mudasir Mohammed Ibrahim">
    ...
    <script type="application/ld+json">
    {
        ...
        "name": "The Official Website of Mudasir Mohammed Ibrahim",
...
<script type="application/ld+json">
{
    "@context": "https://schema.org",
    "@type": "Organization",
    "@id": "https://mudasiribrahim.com/#organization",
    "name": "The Official Website of Mudasir Mohammed Ibrahim",
    "url": "https://mudasiribrahim.com/",
    "logo": "https://raw.githubusercontent.com/mudassiribrahim12/me/main/favicon-32x32.png",
    "description": "Official website of Mudasir Mohammed Ibrahim"
}
</script>
...
<script>
  ...
  (function() {
    ...
    script.onload = function() {
      ...
      // Initialize Tally forms when document is ready
...
.education-item .certificate-link,
.experience-item .certificate-link,
.education-list .certificate-link {
    background-color: #3498db !important;
    color: white !important;
    border: none !important;
}

.education-item .certificate-link:hover,
.experience-item .certificate-link:hover,
.education-list .certificate-link:hover {
    background-color: #3498db !important;
    color: white !important;
    border: none !important;
}
...
.tally-floating-button.pulse {
    animation: pulseAnimation 1.5s ease-in-out infinite;
}

.tally-floating-button.hidden {
    transform: translateY(100px) scale(0.5);
    opacity: 0;
    pointer-events: none;
}

.tally-floating-button .notification-badge {
    position: absolute;
    top: -5px;
    right: -5px;
...
.tally-floating-button:hover .tally-floating-tooltip {
    opacity: 1;
...
/* Auto-popup timer indicator */
.tally-floating-button::after {
    content: '';
    position: absolute;
    top: -5px;
    left: -5px;
...
.tally-floating-button:hover .tally-close-btn {
    opacity: 1;
}
...
@media (max-width: 480px) {
    ...
    color: white !important;
        ...
        /* Target ONLY DOI links - they will stay blue regardless of click/visit status */
a[href*="doi.org"],
a[href*="dx.doi"],
a[href*="doi/"],
.publication-doi a,
.publication-list a[href*="10."],
.publication-list a[href*="doi"] {
...
/* Keep the same blue for all states (link, visited, hover, active) */
a[href*="doi.org"]:link,
a[href*="doi.org"]:visited,
a[href*="doi.org"]:hover,
a[href*="doi.org"]:active,
a[href*="dx.doi"]:link,
a[href*="dx.doi"]:visited,
a[href*="dx.doi"]:hover,
a[href*="dx.doi"]:active,
.publication-doi a:link,
.publication-doi a:visited,
.publication-doi a:hover,
.publication-doi a:active,
.publication-list a[href*="10."]:link,
.publication-list a[href*="10."]:visited,
.publication-list a[href*="10."]:hover,
.publication-list a[href*="10."]:active {
    color: #3498db !important; /* Stay blue */
}

/* Only change on hover (slightly darker blue) */
a[href*="doi.org"]:hover,
a[href*="dx.doi"]:hover,
.publication-doi a:hover,
.publication-list a[href*="10."]:hover {
...
/* Ensure no purple color appears for visited DOIs */
a[href*="doi.org"]:visited,
a[href*="dx.doi"]:visited,
.publication-doi a:visited,
.publication-list a[href*="10."]:visited {
    color: #3498db !important; /* Still blue, not purple */
    opacity: 1;
}
...
/* Specific fix for the about section you mentioned */
.bio-section a {
...
.bio-section a:hover {
    border-bottom: 1px solid var(--nav-hover) !important;
}
...
/* Web of Science - Blue/Teal */
.social-icon[title="Web of Science"]:hover {
    color: #0d5c8c !important; 
}

/* Web of Science - Blue/Teal for profile */
.profile-social-icon[title="Web of Science"]:hover {
    color: #0d5c8c !important;
    background-color: rgba(13, 92, 140, 0.1);
    border-color: #0d5c8c;
}
...
<body style="background-color: var(--bg-color, #ffffff);">
    ...
    <script type="text/babel">
        ...
        const AcademicPortfolio = () => {
            ...
            const FloatingTallyButton = () => {
    const [showButton, setShowButton] = useState(true);
    const [showTooltip, setShowTooltip] = useState(false);
    const [hasInteracted, setHasInteracted] = useState(false);
    const [showAutoPopup, setShowAutoPopup] = useState(false);
    const [autoPopupTimer, setAutoPopupTimer] = useState(15);
    const timerRef = useRef(null);

    // Function to open Tally form
    const openTallyForm = () => {
        if (window.Tally) {
            window.Tally.openPopup('lbrlWB', {
                layout: 'modal',
                width: 700,
                emoji: {
                    text: "👋",
                ...
                autoClose: 5000,
                onOpen: () => {
                    console.log('Tally form opened from floating button');
                    setHasInteracted(true);
                    // Stop auto-popup if user manually opened
                    if (timerRef.current) {
                ...
                onSubmit: (payload) => {
                    ...
                    setShowButton(false); // Hide button after submission
                    setTimeout(() => setShowButton(true), 30000); // Show again after 30 seconds
                }
            });
        }
    };
    ...
    useEffect(() => {
        const startAutoPopupTimer = () => {
            let secondsLeft = 15;
            
            timerRef.current = setInterval(() => {
                secondsLeft -= 1;
                setAutoPopupTimer(secondsLeft);
        ...
        // Start timer after 2 seconds of page load
    ...
    return (
        <button 
            className={`tally-floating-button ${showAutoPopup ? 'pulse' : ''}`}
        ...
        >
            {/* Notification badge showing countdown */}
            {showAutoPopup && !hasInteracted && (
            ...
            {/* Tooltip */}
            {showTooltip && (
                <div className="tally-floating-tooltip">
                    {showAutoPopup ? `Auto-opens in ${autoPopupTimer}s` : 'Quick Feedback Form'}
...
const fetchAppViews = async () => {
    try {
        ...
        const mockViews = {
            "FactorGuard": 16,
            "CalcuStats": 16677,
            "CATrend Analyzer": 7598,
            "ROC Curve Builder": 16010,
            "Data2SPSS": 12849,
            "MedModr": 16068,
            "EpiDem Suite": 17089,
            "ggPubPlot": 14552,
            "APA Table Generator": 3329,
            "SysSampler": 3609,
            "CMH Analyzer": 6853,
            "TNMTC DataLab": 14408,
            "PharmaCalc Pro": 15510,
            "RegEffect Xplorer": 15381,
            "Data TransformR": 16859,
            "CleanMyData": 14660,
            "KMPlot Genie": 16874,
            "QuickStatsGen": 15106,
            "Robust Regressor": 15586,
            "TagSelect": 16725,
            "FAnalyzr": 16885
        };
...
<div className="profile-section">
    ...
    <div style={{ textAlign: 'center', marginBottom: '30px' }}>
        ...
        <div style={{
         ...
         {/* Contact info section with email */}
 ...
 <div className="profile-social-icons" style={{marginTop: '25px', justifyContent: 'flex-start'}}>
    ...
    <a href="https://www.webofscience.com/wos/author/record/HPC-2085-2023" target="_blank" rel="noopener noreferrer" className="profile-social-icon" title="Web of Science">
    ...
    <a href="https://linkedin.com/in/mudasir-mohammed-ibrahim-16b5141b0" target="_blank" rel="noopener noreferrer" className="profile-social-icon" title="LinkedIn">
        ...
        <div style={{
            ...
            <p style={{
                ...
                For any research collaboration, job opportunity,  or just a friendly conversation:
            </p>
            ...
            <div className="bio-section">
                <div className="bio-text">
                    <p>Hello and welcome! I’m Mohammed Mudasir Ibrahim, professionally, Mudasir Mohammed Ibrahim, but “Mudasir Ibrahim” works just fine. Here, you’ll find my research in health and a collection of interactive Shiny web tools I’ve built to make statistical analyses less painful and more fun. Feel free to browse, explore, or email me if you have questions about my apps, research, or anything else, or if you just want to nerd out over data.</p>
                    
                                          
                      <p>By profession, I'm a Registered Nurse at <a href="https://tth.gov.gh">Tamale Teaching Hospital (TTH)</a>. However, I’ve published numerous research articles on nursing education, workforce issues, patient safety, cancer epidemiology, and maternal and child health. Along the way, I review for international journals, and I’ve earned academic reviewer certificates while contributing to the broader scientific community. I enjoy turning real-world clinical challenges into actionable research and practical tools.</p>
                    
<p>One highlight of my work is leading the development of the <a href="https://www.researchgate.net/publication/399881302_Validity_and_Reliability_Study_of_a_Scale_to_Measure_Academic_Help-seeking_Behaviors_Among_Nursing_Students_in_Ghana?_tp=eyJjb250ZXh0Ijp7ImZpcnN0UGFnZSI6ImxvZ2luIiwicGFnZSI6InByb2ZpbGUifX0">Academic Help Seeking Behavior Questionnaire for Nursing Students (AHSBQ-NS)</a>. This instrument was adapted from the Computer Science Help-Seeking Scales (CSHS) by <a href="https://psycnet.apa.org/doi/10.1177/0013164403258447">Pajares et al. (2004)</a> to address a clear gap in Ghanaian nursing education research, providing a context-appropriate way to assess how nursing students seek academic support. To assess your own help-seeking habits as a nursing student or those of your nursing students, access the questionnaire <a href="https://ahsbq-ns.mudasiribrahim.com/">here</a>.</p>
                    
<p>I'm also the creator and owner of <a href="https://newappstesting.shinyapps.io/Data2SPSS/">Data2SPSS</a>, <a href="https://mudassiribrahim2025.shinyapps.io/CalcuStats/">CalcuStats</a>, <a href="https://newappstesting.shinyapps.io/CATrendAnalyzer/">CATrend Analyzer</a>, and <a href="https://mudassiribrahim2025.shinyapps.io/Robustregression/">Robust Regressor</a>, and several other tools (all available under the Software tab). I built these applications using Shiny after noticing the lack of accessible tools and the complexity of existing statistical procedures. My goal is simple: to provide open-access, user-friendly tools that save time, reduce errors, and make statistics less intimidating for researchers using R, regardless of their coding experience. Through these tools, I aim to empower health researchers to focus on insights and decision-making rather than getting lost in technical hurdles.</p>
                ...
                <ul className="experience-list">
                    <li className="experience-item">
                        ...
                        <div className="item-description">Provide evidence-based nursing care to patients with acute and chronic medical conditions, including liver disease, kidney disease, and cancer.</div>
                    </li>
                    <li className="experience-item">
                ...
                <ul className="education-list">
                    ...
                    <li className="education-item">
                        ...
                        <div className="item-period">2025 - Present</div>
                        <div className="item-description">Member</div>
                    </li>
                    <li className="education-item">
...
<div className="education-list" style={{lineHeight: '1.3', marginBottom: '2.5rem'}}>
 <div style={{fontSize: '1rem', color: 'var(--text-color)', marginBottom: '10px'}}>Abubakari Wuni [<a href="https://www.researchgate.net/profile/Abubakari-Wuni" style={{color: 'var(--nav-hover)', textDecoration: 'none'}}>webpage</a>]</div>
<div style={{fontSize: '1rem', color: 'var(--text-color)', marginBottom: '10px'}}>Iddrisu Mohammed Sisala [<a href="https://www.researchgate.net/profile/Iddrisu-Sisala" style={{color: 'var(--nav-hover)', textDecoration: 'none'}}>webpage</a>]</div>
</div>
...
<div style={{fontSize: '1rem', color: 'var(--text-color)', marginBottom: '10px'}}>Abdul Latif Abdul Rahaman [<a href="https://www.researchgate.net/profile/Abdul-Rahaman-7" aria-label="View Abdul Latif Abdul Rahaman's ResearchGate profile" style={{color: 'var(--nav-hover)', textDecoration: 'none'}}>webpage</a>]</div>
<div style={{fontSize: '1rem', color: 'var(--text-color)', marginBottom: '10px'}}>Edna Die [<a href="https://www.researchgate.net/profile/Edna-Die" aria-label="View Edna Die's LinkedIn profile" style={{color: 'var(--nav-hover)', textDecoration: 'none'}}>webpage</a>]</div>
<div style={{fontSize: '1rem', color: 'var(--text-color)', marginBottom: '10px'}}>Abdul-Malik Seidu [<a href="https://www.researchgate.net/profile/Abdul-Malik-Seidu-2" aria-label="View Abdul-Malik Seidu's ResearchGate profile" style={{color: 'var(--nav-hover)', textDecoration: 'none'}}>webpage</a>]</div>
<div style={{fontSize: '1rem', color: 'var(--text-color)', marginBottom: '10px'}}>Alhassan Issahaq [<a href="xxxxx" aria-label="View Alhassan Issahaq's ResearchGate profile" style={{color: 'var(--nav-hover)', textDecoration: 'none'}}>webpage</a>]</div>
</div>
...
<a href="https://1drv.ms/f/c/3ce355b70d76f4f7/Ejkh6TgFPhtNkgsfwRv6_OgBpu9S58AyaqHR_AMPJsR4Rg?e=jU8KMP" className="certificate-link"
                               style={{
                                   display: 'inline-flex',
                                   alignItems: 'center',
                                   gap: '8px',
    ...
    <div className="skills-simple-box">
        ...
        <div className="skill-box">
            ...
            <p style={{color: 'var(--text-color)', lineHeight: '1.6', marginBottom: '15px', fontSize: '1rem'}}>
                ...
                <div className="bio-text">
                    <p>Conducting research, performing statistical analysis, creating web statistical tools, academic/scientific writing, watching movies/dramas, and listening to songs of the 80s and 90s.</p>
                </div>
            </div>
        </div>
...
<a href={link} className="read-post-btn">
...
const ShinyAppsComponent = () => {
    ...
    // Fetch app views when component mounts
    useEffect(() => {
        const fetchAppViews = async () => {
            try {
                setLoadingViews(true);
                
                // Use your mockViews data
                const mockViews = {
                    "FactorGuard": 16,
                    "CalcuStats": 16677,
                    "CATrend Analyzer": 7598,
    ...
    // All apps data WITHOUT views property
    const shinyApps = [
        {
            id: 1,
            name: "FactorGuard",
    ...
    // Combine app data with views from mockViews
    const appsWithViews = shinyApps.map(app => ({
        ...app,
        views: appViews[app.name] || 0
    }));
    
    // Get top viewed apps for display
    const topViewedApps = [...appsWithViews]
    ...
    return (
        <div className="section">
            ...
            {/* Stats sections - Now uses totalViews from state */}
            <div className="apps-stats">
                <div className="stat-card">
                    <div className="stat-number">{shinyApps.length}</div>
                    <div className="stat-label">Applications</div>
                </div>
                <div className="stat-card">
                    <div className="stat-number">{loadingViews ? '...' : totalViews.toLocaleString()}</div>
                ...
                <div className="views-counter-card">
                    <div className="views-counter-header">
                        ...
                        <div className="total-views-container">
                            <div className="total-views-label">Total Application Views (All {shinyApps.length} Apps)</div>
                            <div className="total-views-number">
                                {loadingViews ? 'Loading...' : totalViews.toLocaleString()}
                            </div>
                            <div className="total-views-update">
                                <i className="fas fa-sync-alt" style={{ marginRight: '5px' }}></i>
                                Calculated from individual app views
                            </div>
                        </div>
                    </div>
...
<a href="https://orcid.org/0000-0002-9049-8222" class="social-icon" title="ORCID">
    <i class="fab fa-orcid"></i>
</a>
<a href="https://scholar.google.com/citations?user=xEFzAvgAAAAJ&hl=en" class="social-icon" title="Google Scholar">
    <i class="fas fa-graduation-cap"></i>
</a>
<a href="https://www.webofscience.com/wos/author/record/HPC-2085-2023" class="social-icon" title="Web of Science">
    <i class="fas fa-book-open"></i>
</a>
<a href="https://github.com/mudassiribrahim30" class="social-icon" title="GitHub">
    <i class="fab fa-github"></i>
</a>
<a href="https://linkedin.com/in/mudasir-mohammed-ibrahim-16b5141b0" class="social-icon" title="LinkedIn">
    <i class="fab fa-linkedin"></i>
</a>
<a href="https://www.youtube.com/@mudasirmohammedibrahim1026" class="social-icon" title="YouTube">
...
<div>
    <h3 style={{
        ...
        Measure Your Academic Help-Seeking Behavior
    </h3>
    <p style={{
        color: 'rgba(255, 255, 255, 0.7)',
        fontSize: '0.9rem',
        ...
        Curious about your academic help-seeking habits as a nursing student? Click below to complete the assessment on our website and learn more about your habits.
    </p>
    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
        <a 
            href="https://ahsbq-ns.mudasiribrahim.com/" 
            target="_blank" 
            rel="noopener noreferrer" 
            style={{
                display: 'inline-flex',
...
{/* NEW: Other HTML Web Apps Button - WITH BLUE COLOR */}
<a 
    href="https://apps.mudasiribrahim.com"
    target="_blank"
    rel="noopener noreferrer"
    style={{
        display: 'inline-flex',
        alignItems: 'center',
        gap: '8px',
        background: '#3498db',  /* Changed to solid blue */
        color: 'white',
        padding: '10px 20px',
        borderRadius: '25px',
        textDecoration: 'none',
        ...
        transition: 'all 0.3s ease',
        fontFamily: "'Open Sans', sans-serif",
        border: '1px solid #2980b9'  /* Changed to darker blue border */
    }}
    onMouseOver={(e) => {
        e.target.style.background = '#2980b9';  /* Darker blue on hover */
        e.target.style.transform = 'translateY(-2px)';
        e.target.style.boxShadow = '0 4px 12px rgba(52, 152, 219, 0.3)';
        e.target.style.borderColor = '#1c6ea4';
    }}
    onMouseOut={(e) => {
...
{ icon: 'fab fa-twitter', url: 'https://x.com/WhizMuda', title: 'Twitter (X)', color: '#000000' }
    ...
    <script>
// Floating Tally Form Tab with Auto-Open Feature
class FloatingTallyFormTab {
    constructor() {
        this.hasInteracted = false;
        this.autoPopupTimer = null;
        this.secondsLeft = 15;
        this.isFormOpen = false;
        this.formClosedByUser = false;
    ...
    createFloatingTab() {
        // Create main floating tab container
        this.floatingTab = document.createElement('div');
        this.floatingTab.id = 'floating-tally-tab';
        this.floatingTab.innerHTML = `
            <!-- Floating button (always visible) -->
            <div class="tally-floating-button" id="tally-floating-button">
                <i class="fas fa-comment-alt"></i>
                <div class="tally-notification-badge">15</div>
                <div class="tally-tooltip">Quick Feedback Form<br><small>Form auto-opens on page load</small></div>
            </div>
        `;
    ...
    addStyles() {
        const styles = `
            ...
            /* Floating button styles - BLUE COLOR matching Contact Me button */
            .tally-floating-button {
                width: 60px;
                height: 60px;
                background: linear-gradient(135deg, #3498db, #2980b9);
            ...
            .tally-floating-button .tally-notification-badge {
                position: absolute;
                top: -5px;
                right: -5px;
                background-color: #2ecc71;
            ...
            .tally-floating-button .tally-tooltip {
                position: absolute;
                right: 70px;
                background: rgba(0, 0, 0, 0.9);
                color: white;
            ...
            .tally-floating-button .tally-tooltip small {
                font-size: 0.7rem;
                opacity: 0.8;
                display: block;
                margin-top: 3px;
            ...
            .tally-floating-button:hover .tally-tooltip {
                opacity: 1;
                transform: translateX(0);
            }
            ...
            .tally-form-overlay.active {
                opacity: 1;
                visibility: visible;
            }
            
            /* Tally form container - ALWAYS WHITE even in dark mode */
            ...
            .tally-form-overlay.active .tally-form-container {
                transform: translateY(0) scale(1);
            }
            
            /* Override dark mode for form container */
            body.dark-mode .tally-form-container {
                background: #ffffff !important; /* Force white in dark mode */
                border: 2px solid rgba(52, 152, 219, 0.3); /* Add blue border for visibility */
            }
            
            /* Form header - BLUE COLOR matching button */
            .tally-form-header {
                display: flex;
                justify-content: space-between;
                align-items: center;
            ...
            .tally-form-close:hover {
                background: rgba(255, 255, 255, 0.3);
                transform: rotate(90deg);
            }
            
            /* Timer indicator - only shown in auto-open mode */
            .tally-timer-indicator {
                height: 4px;
                background: #e0e0e0;
                width: 100%;
                overflow: hidden;
            ...
            .timer-mode-indicator.manual-mode {
                background: rgba(46, 204, 113, 0.3);
            }
            
            /* Tally iframe container - ALWAYS WHITE BACKGROUND for form */
            .tally-iframe-container {
                width: 100%;
                height: 450px;
                overflow: hidden;
                background: #ffffff !important; /* Force white background for form */
                position: relative;
            }
            ...
            /* Override dark mode for iframe container */
            body.dark-mode .tally-iframe-container {
                background: #ffffff !important; /* Force white in dark mode */
            }
            
            /* Interactive overlay to detect clicks inside iframe */
            .iframe-interactive-overlay {
                position: absolute;
                top: 0;
                left: 0;
                width: 100%;
            ...
            /* Ensure text inside form container is dark for readability on white background */
            .tally-form-container *:not(.tally-form-header):not(.tally-form-header *):not(.tally-form-close):not(.tally-form-close *) {
            ...
            /* Ensure Tally form loads with white theme */
            .tally-form-overlay {
                /* Add these to ensure Tally form sees white background */
                --tally-background: #ffffff !important;
                --tally-text-color: #333333 !important;
            }
            
            /* Force white theme for Tally iframe */
            #tally-iframe {
    ...
    startAutoOpenTimer() {
        ...
        window.addEventListener('load', () => {
            // Wait 1.5 seconds after page load, then auto-open
            setTimeout(() => {
                if (!this.formClosedByUser) {
                    this.autoOpenMode = true;
                    this.openTallyForm();
                    this.startCountdown();
                }
            }, 1500);
    ...
    startCountdown() {
        // Only start countdown in auto-open mode
        if (!this.autoOpenMode) return;
        
        const badge = this.floatingTab.querySelector('.tally-notification-badge');
        const timerProgress = this.floatingTab.querySelector('.tally-timer-progress');
        const timerModeIndicator = this.floatingTab.querySelector('.timer-mode-indicator');
        
        this.secondsLeft = 15;
        
        // Update timer display
        if (timerModeIndicator) {
            timerModeIndicator.textContent = `⏱️ Auto-closes in ${this.secondsLeft}s`;
            timerModeIndicator.classList.remove('manual-mode');
        }
        
        this.autoPopupTimer = setInterval(() => {
            // Stop timer if user has interacted with form
            if (this.userInteractedWithForm) {
                clearInterval(this.autoPopupTimer);
                
                // Update indicator to manual mode
                if (timerModeIndicator) {
                    timerModeIndicator.textContent = '✓ Manual close';
                    timerModeIndicator.classList.add('manual-mode');
                }
                
                // Hide timer progress bar
                const timerIndicator = this.floatingTab.querySelector('.tally-timer-indicator');
                if (timerIndicator) {
                    timerIndicator.style.opacity = '0.5';
                    timerIndicator.style.height = '2px';
                }
            ...
            // Update badge
            if (badge) {
                badge.textContent = this.secondsLeft;
                
                // Update badge color as time runs out
            ...
            if (timerProgress) {
                const progressPercent = (this.secondsLeft / 15) * 100;
            ...
            if (this.secondsLeft <= 0) {
                ...
                // Update badge
                if (badge) {
                    badge.innerHTML = '<i class="fas fa-redo"></i>';
                    badge.style.backgroundColor = '#3498db';
                    badge.style.animation = 'none';
                    badge.title = 'Click to reopen form';
                ...
                // Update tooltip
                const tooltip = this.floatingTab.querySelector('.tally-tooltip');
                if (tooltip) {
                    tooltip.innerHTML = 'Quick Feedback Form<br><small>Click to reopen the form</small>';
                }
            }
        }, 1000);
    }
    ...
    openTallyForm() {
        ...
        // Create overlay and form container
        ...
        const floatingButton = this.floatingTab.querySelector('.tally-floating-button');
        floatingButton.style.transform = 'scale(1.1)';
        floatingButton.style.boxShadow = '0 6px 25px rgba(52, 152, 219, 0.6)';
        
        // Update tooltip
        const tooltip = this.floatingTab.querySelector('.tally-tooltip');
        if (tooltip) {
            if (this.autoOpenMode) {
                tooltip.innerHTML = 'Quick Feedback Form<br><small>Form auto-opened - 15s timer active</small>';
            } else {
                tooltip.innerHTML = 'Quick Feedback Form<br><small>Form opened manually</small>';
            }
        }
    }
    ...
    createFormOverlay() {
        ...
        const existingOverlay = document.querySelector('.tally-form-overlay');
        if (existingOverlay) {
            existingOverlay.remove();
        }
        
        // Determine header content based on mode
        let timerIndicatorHtml = '';
        if (this.autoOpenMode) {
            timerIndicatorHtml = `<span class="timer-mode-indicator">⏱️ Auto-closes in 15s</span>`;
        } else {
            timerIndicatorHtml = `<span class="timer-mode-indicator manual-mode">✓ Manual close</span>`;
        }
        
        // Create new overlay with proper title
        const overlay = document.createElement('div');
        overlay.className = 'tally-form-overlay';
        overlay.innerHTML = `
            <div class="tally-form-container">
                <div class="tally-form-header">
                    <div style="flex: 1;">
                        ...
                        <div class="form-subtitle">Share your feedback or suggestions</div>
                    </div>
                    <button class="tally-form-close" id="tally-form-close-btn">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                ${this.autoOpenMode ? '<div class="tally-timer-indicator"><div class="tally-timer-progress" style="width: 100%"></div></div>' : ''}
                <div class="tally-iframe-container" id="tally-iframe-container">
                    <!-- Interactive overlay to detect clicks -->
                    <div class="iframe-interactive-overlay" id="iframe-interactive-overlay"></div>
                    <!-- Tally form will be loaded here -->
                </div>
            </div>
        `;
    ...
    loadTallyForm() {
        const iframeContainer = document.getElementById('tally-iframe-container');
        if (!iframeContainer) return;
        
        // Create iframe with your Tally form - force white theme
        const iframe = document.createElement('iframe');
        // Add transparentBackground=1 and theme=light to ensure white background
        iframe.src = 'https://tally.so/embed/lbrlWB?alignLeft=1&transparentBackground=1&theme=light';
        iframe.title = 'Quick Feedback Form';
        ...
        interactiveOverlay.className = 'iframe-interactive-overlay';
        interactiveOverlay.id = 'iframe-interactive-overlay';
        iframeContainer.appendChild(interactiveOverlay);
        
        // Set up click detection on the overlay
        setTimeout(() => {
    ...
    setupFormInteractionDetection() {
        const interactiveOverlay = document.getElementById('iframe-interactive-overlay');
        const iframe = document.getElementById('tally-iframe');
        
        if (!interactiveOverlay || !iframe) return;
        
        // Remove overlay after a short delay to allow clicks to pass through
        setTimeout(() => {
            if (interactiveOverlay && interactiveOverlay.parentNode) {
                interactiveOverlay.style.pointerEvents = 'none';
            }
        }, 1000);
        
        // Track clicks on the overlay (initial protection layer)
        interactiveOverlay.addEventListener('click', () => {
            this.handleFormInteraction();
        });
        
        // Track clicks on the form container (fallback)
        const formContainer = document.querySelector('.tally-form-container');
        if (formContainer) {
            formContainer.addEventListener('click', (e) => {
                // Only track clicks inside the iframe area
                const iframeContainer = document.querySelector('.tally-iframe-container');
                if (iframeContainer && iframeContainer.contains(e.target)) {
                    this.handleFormInteraction();
                }
            });
        }
        
        // Track focus events on the iframe
        iframe.addEventListener('load', () => {
            try {
                // Try to access iframe content to set up event listeners
                ...
                // Cross-origin restrictions may prevent this, but we have other methods
                console.log('Form interaction tracking enabled');
            }
        });
        
        // Track mouse enter on iframe container
        const iframeContainer = document.querySelector('.tally-iframe-container');
        if (iframeContainer) {
            iframeContainer.addEventListener('mouseenter', () => {
                // Mark interaction if mouse enters the form area (user is likely to interact)
                setTimeout(() => {
                    this.handleFormInteraction();
                }, 500);
            });
    ...
    handleFormInteraction() {
        // Only relevant for auto-open mode with timer
        if (!this.autoOpenMode || this.userInteractedWithForm) return;
        
        this.userInteractedWithForm = true;
        ...
        const timerModeIndicator = document.querySelector('.timer-mode-indicator');
        if (timerModeIndicator) {
            timerModeIndicator.textContent = '✓ Manual close';
            timerModeIndicator.classList.add('manual-mode');
        }
        ...
        const timerIndicator = document.querySelector('.tally-timer-indicator');
        if (timerIndicator) {
            timerIndicator.style.opacity = '0.5';
            timerIndicator.style.height = '2px';
        }
        ...
        const tooltip = this.floatingTab.querySelector('.tally-tooltip');
        if (tooltip) {
            tooltip.innerHTML = 'Quick Feedback Form<br><small>Manual close required</small>';
        }
    }
    ...
    closeTallyForm() {
        ...
        if (floatingButton) {
            ...
            floatingButton.style.boxShadow = '0 4px 20px rgba(52, 152, 219, 0.4)';
        }
        ...
        // Mark as closed by auto-timer (not user)
        if (this.secondsLeft <= 0) {
            this.formClosedByUser = false;
        }
        
        // Reset auto-open mode for next opening
        this.autoOpenMode = false;
        this.userInteractedWithForm = false;
    }
    ...
    setupEventListeners() {
        ...
        // Open form on button click (manual open - no timer)
        floatingButton.addEventListener('click', (e) => {
            e.stopPropagation();
            
            if (this.isFormOpen) {
                this.closeTallyForm();
                this.formClosedByUser = true;
            } else {
                // Manual open - no auto-close timer
                this.autoOpenMode = false;
                this.openTallyForm();
                this.formClosedByUser = false;
            }
        ...
        document.addEventListener('click', (e) => {
            const overlay = this.overlay;
            if (this.isFormOpen && overlay && e.target === overlay) {
                this.closeTallyForm();
                this.formClosedByUser = true;
            }
        });
        ...
        document.addEventListener('keydown', (e) => {
            if (this.isFormOpen && e.key === 'Escape') {
                this.closeTallyForm();
                this.formClosedByUser = true;
            }
        });
        
        // Handle form close button (delegated since it's dynamically created)
        document.addEventListener('click', (e) => {
            if (e.target.closest('#tally-form-close-btn')) {
                this.closeTallyForm();
                this.formClosedByUser = true;
            }
        });
        ...
        window.addEventListener('message', (event) => {
            if (event.data.type === 'tally.formSubmitted') {
                ...
                if (badge) {
                    ...
                    // Keep checkmark for 5 seconds
                    setTimeout(() => {
                        badge.textContent = '✓';
                        badge.title = 'Form submitted successfully';
                    }, 5000);
                }
                ...
                const tooltip = this.floatingTab.querySelector('.tally-tooltip');
                if (tooltip) {
                    tooltip.innerHTML = 'Quick Feedback Form<br><small>Thank you for your feedback!</small>';
                }
            }
        });
    }
...
// Initialize the floating Tally form tab when page loads
document.addEventListener('DOMContentLoaded', () => {
    // Remove any existing Tally floating button
    const existingButton = document.querySelector('.tally-floating-button');
    if (existingButton && existingButton.parentNode) {
        existingButton.parentNode.remove();
    }
    
    // Create new floating tab
    new FloatingTallyFormTab();
});
</script>
</body>
</html>
